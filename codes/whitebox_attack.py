...
Outputs:
  ...
  - white-box attack success rate (over >= 1000 correctly classified test images)
  - 10 randomly chosen successful (orig, adv) pairs as PNG images, plus a
    JSON manifest containing the classifier's prediction for each.
"""
import os
import json
import random
...
# Fashion-MNIST class names (for nicer figures)
CLASS_NAMES = [
...
def collect_correct(classifier, loader, device, num=1000):
    """Collect at least `num` test images that the classifier predicts correctly."""
    classifier.eval()
    xs, ys = [], []
    with torch.no_grad():
        for x, y in loader:
...
def targeted_pgd(classifier, x, y_target, device,
    ...
    """
    Targeted PGD on raw uint8-scale Fashion-MNIST images (pixel range [0, 255]).
    eps:    L_inf perturbation budget (in pixel units, /255 for the usual scale)
    alpha:  step size per iteration (pixel units)
    steps:  number of iterations
    Returns: adversarial tensor of same shape as x.
    """
    classifier.eval()
    ...
    for _ in range(steps):
        ...
        # targeted: descend the gradient w.r.t. target loss
        with torch.no_grad():
            x_adv = x_adv - alpha * grad.sign()
            # project back into L_inf eps ball around x_orig
...
def save_sample_grid(originals, adversarials, orig_preds, adv_preds, out_path):
    ...
    for i in range(n):
        ...
        ax.set_title(f"orig: {CLASS_NAMES[orig_preds[i].item()]}", fontsize=8)
        ax.axis("off")
        ax = axes[1, i]
        ax.imshow(adversarials[i].reshape(28, 28).numpy(), cmap="gray", vmin=0, vmax=255)
        ax.set_title(f"adv: {CLASS_NAMES[adv_preds[i].item()]}", fontsize=8)
        ax.axis("off")
...
def main():
    ...
    if not os.path.exists(args.ckpt):
        # fall back to provided checkpoint
        args.ckpt = "../model/cnn.ckpt"
    ...
    # 1) collect >=1000 correctly classified test images
    x_correct, y_correct = collect_correct(classifier, test_loader, device, num=args.num_samples)
    print(f"[info] collected {x_correct.shape[0]} correctly classified test images")

    # 2) targeted attack with mapping label -> (label + 1) % 10
    # 0->1 1->2 2->3 4->5 5->6 6->7 7->8 8->9 9->0
    ...
    # 3) random 10 successful pairs -> images + manifest
    success_idx = torch.nonzero(success_mask, as_tuple=False).flatten().tolist()
    ...
    sel_adv = adv[pick]
    sel_orig_pred = y_correct[pick]   # original label = original prediction (we filtered by correctness)
